...
class XCBuildConfiguration(PBXObject):
	...
	def addOtherCFlag (self, flag):
		

		if flag == None :
			return

		flags = self.getBuildSetting("OTHER_CFLAGS")
		...
		if flags == None :
			...
			if isinstance(flags,str):
				...
				flagArray = []
				# 把旧字符串填充进去
				flagArray.append(flags)

				flags = flagArray
				#self.setBuildSetting("OTHER_CFLAGS", flagArray)
			# else:
				# flags.append(flag)
				#self.setBuildSetting("OTHER_CFLAGS", flags)

		flags.append(flag)
		self.setBuildSetting("OTHER_CFLAGS", flags)

	def removeBuildSettings(self, key):

		if key == None:
			return

		if self.data["buildSettings"].has_key(key):
			del	self.data["buildSettings"][key]


	def addUserDefineSettingWithString(self, key, stringValue):
		...
		self.setBuildSetting(key, stringValue)
