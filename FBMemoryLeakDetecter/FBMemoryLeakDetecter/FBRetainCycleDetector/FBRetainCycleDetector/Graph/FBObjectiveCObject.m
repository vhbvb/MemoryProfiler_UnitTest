    ...
    for (NSInteger i = 0; i < tries; ++i) {
      ...
      @try {
        ...
        for (id subobject in self.object) {
            ...
            NSString *key = nil;
            if ([self.object isKindOfClass:NSMapTable.class])
            {
                key = [[self.object keyPointerFunctions] descriptionFunction]((__bridge const void * _Nonnull)(subobject));
            }
            
          if (retainsKeys) {
              
              FBObjectiveCGraphElement *element = FBWrapObjectGraphElement(self, key?:subobject, self.configuration);
          ...
          if (isKeyValued && retainsValues) {
            ...
            FBObjectiveCGraphElement *element = FBWrapObjectGraphElement(self,
                                                                         [self.object objectForKey:key?:subobject],
                                                                         self.configuration);
            if (element) {
              [temporaryRetainedObjects addObject:element];
            }
